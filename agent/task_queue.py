...
class TaskQueue:
    def __init__(self, max_concurrent: int = 1):
        ...
        self._executor       = None
        # Optional UI log callback: (text: str) -> None
        self._log_callback: Callable[[str], None] | None = None

    def set_log_callback(self, cb: Callable[[str], None]) -> None:
        """Inject a UI log function so task events appear in the activity log."""
        self._log_callback = cb

    def _log(self, msg: str) -> None:
        """Write to stdout and, if available, the UI log."""
        print(msg)
    ...
    def start(self) -> None:
        ...
        self._log("[TaskQueue] ✅ Worker started")
    ...
    def submit(
        ...
        self._log(f"SYS: Task queued [{task_id}]: {goal[:60]}")
        return task_id
    ...
    def _run_task(self, task: Task) -> None:
        self._log(f"SYS: Task [{task.task_id}] started: {task.goal[:60]}")
        try:
            ...
            with self._lock:
                if task.cancel_flag.is_set():
                    task.status = TaskStatus.CANCELLED
                    self._log(f"SYS: Task [{task.task_id}] cancelled.")
                else:
                    task.status = TaskStatus.COMPLETED
                    task.result = result
                    self._log(f"SYS: Task [{task.task_id}] done: {str(result)[:80]}")
                self._active_count -= 1
            ...
            with self._lock:
                ...
                task.error  = str(e)
                self._active_count -= 1
            self._log(f"ERR: Task [{task.task_id}] failed: {str(e)[:100]}")
