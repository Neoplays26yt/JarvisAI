...
def _run_generated_code(description: str, speak: Callable | None = None) -> str:
    msg = "ERROR: Arbitrary code execution (generated_code) is disabled for safety reasons. You must use the built-in tools. If a built-in tool cannot accomplish this task, you must gracefully fail or ask for user confirmation."
    print(f"[Executor] 🛑 Blocked arbitrary code execution: {description[:50]}")
    return msg
...
def _inject_context(params: dict, tool: str, step_results: dict, goal: str = "") -> dict:
    ...
    if tool == "file_controller" and params.get("action") in ("write", "create_file"):
        ...
        if not content or len(content) < 50:
            ...
            if all_results:
                # TRUNCATE injected results to avoid token explosion
                combined = "\n\n---\n\n".join(all_results)[:2000]
                translated = _translate_to_goal_language(combined, goal)
                params["content"] = translated
                print(f"[Executor] 💉 Injected + translated content (truncated)")
...
def _call_tool(tool: str, parameters: dict, speak: Callable | None) -> str:
    ...
    elif tool == "github_automation":
        from actions.github_automation import github_automation
        return github_automation(parameters=parameters, player=None) or "Done."
