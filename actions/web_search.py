...
def _format_ddg(query: str, results: list[dict]) -> str:
    ...
    for i, r in enumerate(results, 1):
        ...
        if r.get("url"):     lines.append(f"Link: {r['url']}")
        lines.append("")
    return "\n".join(lines).strip()
...
def web_search(
    ...
    if visual:
        try:
            ...
            auto_close = params.get("auto_close", 8)
            print(f"[WebSearch] 👀 Visual search requested. Auto-close: {auto_close}s")
            
            # Start visual search in background so we don't block Gemini results
            import threading
            def _vis_search():
                browser_control({
                    "action": "search",
            ...
            threading.Thread(target=_vis_search, daemon=True).start()
        except Exception as e:
            print(f"[WebSearch] ⚠️ Failed to start visual search: {e}")
