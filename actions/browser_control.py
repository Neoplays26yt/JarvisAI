...
def browser_control(
    ...
    try:
        ...
        if action in ("go_to", "search") and params.get("auto_close", 0):
            delay = int(params.get("auto_close", 0))
            if delay > 0:
                threading.Timer(delay, lambda: _registry.close_one(sess.browser_name)).start()
                result += f" (Auto-closing in {delay}s)"
